module Jekyll
  ...
  class Site
    ...
    attr_accessor :post_filters, :site_filters
    ...
    def load_site_filters
      self.site_filters = Jekyll::SiteFilter.descendants.select do |c|
        !self.safe || c.safe
      end.map do |c|
        c.new(self.config)
      end
    ...
    def post_render
      self.load_site_filters unless self.site_filters

      if self.site_filters
        self.site_filters.each do |filter|
